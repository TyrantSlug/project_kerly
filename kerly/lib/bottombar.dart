...
class _BottomBarState extends State<BottomBar> {
  ...
  Future<void> loadBookmarkData() async {
    try {
      String url =
          'http://15.164.56.106:9007/bookmark-service/bid/${widget.id}/username/${Provider.of<MemberModel>(context, listen: false).username}';
      final response = await http.get(
        Uri.parse(url),
        headers: {"Content-Type": "application/json"},
      );
  ...
  Future<void> processCreateBookmark() async {
    ...
    try {
      String url = 'http://15.164.56.106:9007/bookmark-service/createBookmark';
  ...
  Future<void> processDeleteBookmark() async {
    try {
      String url = 'http://15.164.56.106:9007/bookmark-service/deleteBookmark';
              ...
              children: [
                ElevatedButton(
                  onPressed: isLoggedIn ? () {
                    // 장바구니 추가 버튼 클릭 시 동작
                    if (!isAddedToCart) {
                      Provider.of<ShoppingCartModel>(context, listen: false).addToCart(_product);
