...
class _ItemDetailPageState extends State<ItemDetailPage>
  ...
  Future<void> loadData() async {
    ...
    try {
      String url = 'http://15.164.56.106:9007/item-service/id/${widget.id}';
    ...
    if (!_imageMap.containsKey(bid)) {
      try {
        var response = await http.get(
            Uri.parse('http://15.164.56.106:9007/file-service/fileDownload/$bid'));
        if (response.statusCode == 200) {
          setState(() {
  ...
  void processDelete(id) async {
    ...
    String url = "http://15.164.56.106:9007/item-service/item/admin";
          ...
          child: SingleChildScrollView(
            child: isLoading
              ...
              children: [
                ...
                Text("상품 설명: $_itemDescribe", style: TextStyle(fontSize: 20)),
                Text("상품 종류: $_itemType", style: TextStyle(fontSize: 20)),
                Text("판매자: $_username", style: TextStyle(fontSize: 20)),
                Text("수량: $_ea", style: TextStyle(fontSize: 20)),
                Text("정가: $_price", style: TextStyle(fontSize: 20)),
                Text("할인률: $_discount%", style: TextStyle(fontSize: 20)),
                Text("최종가: $_totalPrice", style: TextStyle(fontSize: 25)),
                SizedBox(
                  height: 10,
                ),
                Text(
                  " 상품 등록일: $_createDate".substring(0, 19),
                  style: TextStyle(fontSize: 15),
                ),
                Text(
                  " 상품 수정일: $_updateDate".substring(0, 19),
                  style: TextStyle(fontSize: 15),
                ),
                SizedBox(height: 20),
                if (isOwner) // 소유자인 경우에만 표시
